...
app.get('/api/pkgs', (req, res) => {
  ...
  if (!basePath || !fs.existsSync(basePath)) {
    return res.json({ pkgs: [], error: basePath ? 'Path does not exist' : 'No base path configured' });
  }
...
app.post('/api/send', async (req, res) => {
  ...
  if (!config.ps4ip) {
    return res.status(400).json({ success: false, error: 'PS4 IP not configured' });
  }

  if (!pkgPath || !fs.existsSync(pkgPath)) {
    return res.status(400).json({ success: false, error: 'PKG file not found' });
  }
